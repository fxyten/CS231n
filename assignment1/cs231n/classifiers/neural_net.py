...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        f1 = np.maximum(X.dot(W1) + b1, 0)#方便后面反向传播
        scores = f1.dot(W2) + b2
        ...
        loss = np.sum(- np.log(scores[range(N), y] / np.sum(scores, axis=1)))
        loss /= N
        ...
        ds[range(N), y] -= 1
        ds /= N
        grads['W2'] = f1.T.dot(ds) + 2 * reg * W2
        grads['b2'] = np.sum(ds, axis=0)
        
        df1 = ds.dot(W2.T)
        relu_idx = np.zeros_like(f1)
        relu_idx[f1>0] = 1
        df1_relu = df1 * relu_idx
        grads['W1'] = X.T.dot(df1_relu) + 2 * reg * W1

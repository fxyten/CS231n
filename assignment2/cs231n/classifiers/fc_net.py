...
class TwoLayerNet(object):
    ...
    def __init__(
        ...
        # 第二步：构造权重字典，并进行w1、b1、w2、b2权重初始化
        ############################################################################
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(2, num_layers):  # 最后一层只有全连接没有relu
            out[str(i)], cache[str(i)] = affine_bn_ln_relu_dp_forward(out[str(i - 1)], self.params['W' + str(i)],
                                                                      self.params['b' + str(i)],
        ...
        scores, cache[str(num_layers)] = affine_forward(out[str(num_layers - 1)], self.params['W' + str(num_layers)],
        ...
        dout[str(num_layers)], grads['W' + str(num_layers)], grads['b' + str(num_layers)] = affine_backward(dscores,
                                                                                                            cache[str(
                                                                                                                num_layers)])

        for i in range(2, num_layers)[::-1]:  # [::-1]坐标倒序
            dout[str(i)], grads['W' + str(i)], grads['b' + str(i)], grads['gamma' + str(i)], grads[
                'beta' + str(i)] = affine_bn_ln_relu_dp_backward(dout[str(i + 1)], cache[str(i)])

        _, grads['W1'], grads['b1'], grads['gamma1'], grads['beta1'] = affine_bn_ln_relu_dp_backward(dout['2'],
                                                                                                     cache['1'])  # 第一层

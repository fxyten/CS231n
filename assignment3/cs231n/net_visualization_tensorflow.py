...
def compute_saliency_maps(X, y, model):
    ...
    with tf.GradientTape() as tape:
        tape.watch(tf_X)
        correct_scores = tf.gather_nd(model(tf_X), tf.stack((tf.range(X.shape[0]), y), axis=1))
        saliency = tape.gradient(correct_scores, tf_X)
...
def make_fooling_image(X, target_y, model):
    ...
    while True:
      with tf.GradientTape() as tape:
        ...
        y = np.argmax(scores)
        print("target; ", target_y, "  rez: ",y)
        if y == target_y:
          return X_var

        # Score for the target class.
        target_score = scores[0,target_y]
        saliency = tape.gradient( target_score, X_var)
        saliency = learning_rate * saliency / np.linalg.norm(saliency)
        X_var = X_var + saliency
...
def class_visualization_update_step(X, model, target_y, l2_reg, learning_rate):
    ...
    with tf.GradientTape() as tape:
      ...
      scores = model(X)
      target_score = scores[0,target_y]
      r = tape.gradient( target_score, X)
      # 正则化的作用：周期性模糊图片
      r -= 2*l2_reg*X  # Add l2 reg derivative.

      X = X + learning_rate * r / np.linalg.norm(r)
